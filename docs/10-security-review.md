# bank0 - Security model

**TL;DR.** Two surfaces with two authentication schemes that are never
interchangeable: the client API takes a JWT bearer, the portal takes a cookie
session. A valid session is not authorization - every admin mutation carries its
own role gate. Ownership scoping answers 404, not 403, so the API never confirms
that another customer's account exists. Everything here is covered by a named
test.
...
Every request crosses the same outer middleware before it reaches a surface-specific
gate. The order matters: the security headers and the timeout apply even to
requests that are about to be rejected.

```mermaid
flowchart LR
    R([request]) --> L[requestLogger]
    L --> RC[recoverer]
    RC --> SH[securityHeaders]
    SH --> TO[timeout]
    TO --> P{route}
    P -->|/auth/* public| RL[rateLimit per IP]
    P -->|client subrouter| J[requireJWT]
    P -->|portal subrouter| SE[requireSession] --> CS[csrfGuard]
    RL --> H[handler]
    J --> H
    CS --> H
    H --> FN[one DB function]
    FN --> ME[mapDBError]
```

Diagram: logging, panic recovery, security headers and the request timeout wrap
every route. Public auth routes then pass through a per-IP rate limiter; the
client subrouter through `requireJWT`; the portal subrouter through
`requireSession` followed by the CSRF same-origin guard. Each handler calls one
database function, and `mapDBError` turns a SQLSTATE into a status code.
...
These describe what the system does not do, rather than work queued to do.
