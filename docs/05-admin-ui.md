# bank0 - Operator console

**TL;DR.** Server-rendered Go Templ and HTMX behind a cookie session, organized
around three ideas: roles gate every mutation individually, dangerous money moves
need a second operator (maker-checker - the person who creates a large transfer
cannot approve it), and every action is attributed in an immutable audit table.
A valid session is authentication, never authorization.
...
- **Queue** (`GET /console/disputes` -> `/console/disputes/results`): each row shows
  ...
  the status filter).
- **Decide / Recall (JSON)**: `POST /admin/disputes/{id}/decide`
...
> a valid session. Money, account and dispute mutations require `canActOnMoney`.
> User management - creating a user over JSON (`POST /users`), changing a role or
> status - requires `canManageUsers`, which is **admin only**. The `canCreateUsers`
> gate (operator or admin) applies to the *console* create-user form and to editing
> a user's invite quota (`POST /console/users/{id}/invites`, audited as
> `set_invites`), not to the JSON endpoint. Reads stay open to any staff. See
> [`10-security-review.md`](10-security-review.md).

> **Forced password rotation binds both surfaces.** Requiring a password change
> signs the customer out and stops their client-API access too: their next login
> returns `password_change_required` and a token that reaches only
> `POST /me/password` ([`06-client-api.md`](06-client-api.md) §2.1). Before that
> fix the flag bound the console alone, which customers cannot reach.

### 4.8 Warning rules (fraud policy)
...
pattern: the panel ships the search box and the container, the rows fragment is
what HTMX swaps back in. There is no confirm-modal component
