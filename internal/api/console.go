...
func (s *Server) registerConsole(r *mux.Router) {
	...
	r.HandleFunc("/console/dashboard", s.page(s.consoleDashboard)).Methods(http.MethodGet)
	r.HandleFunc("/console/users", s.page(s.consoleUsers)).Methods(http.MethodGet)
	r.HandleFunc("/console/accounts", s.page(s.consoleAccounts)).Methods(http.MethodGet)
	r.HandleFunc("/console/pending", s.page(s.consoleTransfers)).Methods(http.MethodGet)
	r.HandleFunc("/console/reconcile", s.page(s.consoleReconcile)).Methods(http.MethodGet)
	r.HandleFunc("/console/audit", s.page(s.consoleAudit)).Methods(http.MethodGet)
	r.HandleFunc("/console/approvals", s.page(s.consoleApprovals)).Methods(http.MethodGet)
	r.HandleFunc("/console/limit-requests", s.page(s.consoleLimitRequests)).Methods(http.MethodGet)
	r.HandleFunc("/console/disputes", s.page(s.consoleDisputes)).Methods(http.MethodGet)
	r.HandleFunc("/console/warning-rules", s.page(s.consoleWarningRules)).Methods(http.MethodGet)
	r.HandleFunc("/console/watchlist", s.page(s.consoleWatchlist)).Methods(http.MethodGet)
	r.HandleFunc("/console/settings", s.page(s.consoleSettings)).Methods(http.MethodGet)
